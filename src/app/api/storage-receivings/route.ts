...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedStorageReceivings = storageReceivings.map(receiving => ({
      ...
      receivingOrderId: receiving.receivingOrderId ? receiving.receivingOrderId.toString() : null,
      receivingNumber: receiving.receivingNumber,
      warehouseId: receiving.warehouseId.toString(),
      pickFromWarehouseId: receiving.pickFromWarehouseId ? receiving.pickFromWarehouseId.toString() : null,
      storagePickId: receiving.storagePickId ? receiving.storagePickId.toString() : null,
    ...
    return NextResponse.json({ message: error.message || 'Failed to fetch storage receiving records' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      await storageReceiving.save({ session: session_db });

      // Process pallets if provided
      if (body.pallets && Array.isArray(body.pallets) && body.pallets.length > 0) {
        // Prepare pallet data with the new storage receiving ID
        const palletData = body.pallets.map((pallet: any) => ({
          ...pallet,
        ...
        // Create all pallets
        await TrnStorageReceivingPallet.insertMany(palletData, { session: session_db });
      }
    ...
    return NextResponse.json({ message: error.message || 'Failed to create storage receiving' }, { status: 500 });
  }
}