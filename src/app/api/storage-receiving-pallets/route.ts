...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedStorageReceivingPallets = storageReceivingPallets.map(pallet => ({
    ...
    return NextResponse.json({ message: error.message || 'Failed to fetch storage receiving pallets' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: 'Storage receiving pallet created successfully', id: storageReceivingPallet._id.toString() }, { status: 201 });
  } catch (error: any) {
    console.error('Error creating storage receiving pallet:', error);
    return NextResponse.json({ message: error.message || 'Failed to create storage receiving pallet' }, { status: 500 });
  }
}